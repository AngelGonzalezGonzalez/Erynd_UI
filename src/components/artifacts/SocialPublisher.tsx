...
import { Button, Tip } from '../primitives';
...
export function SocialPublisher({ full, state }: ArtifactProps) {
  ...
  // "Tighten copy" — brief processing, then a shorter, on-brand version.
  const tighten = useAsyncAction(
    () => setText('Battery longevity matters — so we’re fixing it fast. Aria 2 firmware 1.4.2 ships Thursday; affected units get a no-questions replacement starting today.'),
    { successToast: t('toast.copyTightened'), minMs: 700 }
  );
  ...
  const composeBlocked = overLimit.length > 0 || on.length === 0;
    ...
    <div className={a.body}>
      <div>
        ...
        <div className={a.platformToggle}>
          {platforms.map((p) => (
            <Tip key={p.id} text={`${t('tip.postTo')} ${p.label} · ${p.limit}`} side="top">
              <button className={`${a.platBtn} ${on.includes(p.id) ? a.platOn : ''}`} aria-pressed={on.includes(p.id)} onClick={() => toggle(p.id)}>{p.label}</button>
            </Tip>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <textarea className={`${a.field} ${a.textarea}`} aria-label={t('pub.compose')} value={text} onChange={(e) => setText(e.target.value)} />
        ...
        <div className={a.actions} style={{ marginTop: 'var(--sp-2)' }}>
          <Tip text={t('tip.aiAssist')} side="top">
            <Button variant="ghost" size="sm" loading={tighten.loading} onClick={tighten.run}>◎ {t('pub.aiAssist')}</Button>
          </Tip>
          <Tip text={t('tip.hashtags')} side="top">
            <Button variant="ghost" size="sm" onClick={() => { setText((x) => x + ' #Aria2 #NuveraSound'); pushToast(t('toast.hashtags')); }}>◎ {t('pub.hashtags')}</Button>
          </Tip>
        </div>
      </div>
      ...
      {status === 'idle' ? (
        <div className={a.actions}>
          <Tip text={composeBlocked ? t('tip.composeDisabled') : t('tip.schedule')} side="top">
            <Button variant="primary" size="sm" onClick={schedule} disabled={composeBlocked}>{t('common.approve')} · {t('pub.schedule')}</Button>
          </Tip>
          <Tip text={composeBlocked ? t('tip.composeDisabled') : t('tip.publishNow')} side="top">
            <Button variant="secondary" size="sm" onClick={schedule} disabled={composeBlocked}>{t('pub.publishNow')}</Button>
          </Tip>
        </div>
      ) : (
        <div>
